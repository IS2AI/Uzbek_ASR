...
if [ ! -f path.sh ] || [ ! -f cmd.sh ]; then
    echo "Please change current directory to recipe directory e.g., egs/Uzbek_ASR/asr1"
    exit 1
fi
...
# rnnlm related
use_lang_model=true
lang_model=exp/train_rnnlm_pytorch_lm/rnnlm.model.best #if use_lang_model=true, path to rnnlm.model.best

# decoding parameter
cmvn=data/train/cmvn.ark #path to cmvn.ark for example data/train/cmvn.ark
recog_model=exp/train_pytorch_train_pytorch_conformer_lr5_specaug/results/model.last10.avg.best  #path to model.last10.avg.best 
